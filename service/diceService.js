...
export const generateCoCAttributes = () => {
  ...
  const pow = roll3d6x5();
  return {
    STR: roll3d6x5(), CON: roll3d6x5(), DEX: roll3d6x5(), APP: roll3d6x5(),
    POW: pow, LUCK: roll3d6x5(), SIZ: roll2d6plus6x5(), INT: roll2d6plus6x5(),
    EDU: roll2d6plus6x5(), SAN: pow
  };
};
...
export const parseDeck = (template) => {
  ...
  result = result.replace(/(\d*d\d+([+-]\d+)?)/gi, (match) => rollDice(match).total.toString());
  return result;
};
