...
const App = () => {
  ...
  const handleCommand = useCallback((raw) => {
    ...
    if (lower.startsWith('.nn ')) {
      ...
      if (newName) {
        ...
        const newState = { 
          ...appState, 
          users: appState.users.map(u => u.email === user.email ? updatedUser : u) 
        };
        saveState(newState);
  ...
  const handleLogin = (u) => {
    ...
    saveState({ 
      users: existing 
        ? existingUsers.map(x => x.email === u.email ? updatedUser : x) 
        : [...existingUsers, updatedUser] 
    });
  };

  if (!user) {
    return <Login onLogin={handleLogin} bannedEmails={appState.config.bannedEmails} />;
  }
      ...
      <header className="fixed top-0 left-0 right-0 h-20 bg-white/70 backdrop-blur-xl z-30 px-8 flex items-center justify-between border-b border-amber-100/30 transition-all">
        <div className="flex items-center gap-4">
          {appState.config.logoImage ? (
             <img src={appState.config.logoImage} className="w-10 h-10 rounded-2xl object-cover shadow-lg shadow-amber-200/50" alt="Logo" />
          ) : (
             <div className="w-10 h-10 rounded-2xl bg-amber-500 flex items-center justify-center text-white font-bold shadow-lg shadow-amber-200/50">锦</div>
          )}
          <div>
